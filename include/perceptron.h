...
public:
    ...
    double forward (pair<vector<double>, int>&); // Une fonction membre forward qui prend en paramètre un Input (de préférence par référence), et qui renvoie une valeur réelle (double) correspondant à l’application de l’algorithme du perceptron à l’input en paramètre (correspond au calcul de 𝒜(𝒙)).
    double calcul_delta (pair<vector<double>, int>&);
    void backprop (pair<vector<double>, int>&, double mu); // Une fonction membre backprop qui prend en paramètre un Input (de préférence par référence), ainsi qu’une valeur réelle (double) correspondant au pas de gradient 𝜇, et qui ne renvoie rien. Cette fonction va apprendre les poids à partir de l’input en paramètre en utilisant la formule \
